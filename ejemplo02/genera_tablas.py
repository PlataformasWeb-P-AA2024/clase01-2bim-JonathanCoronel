...
class Provincia(Base):
    ...
    def obtner_lista_parroquias(self):
        cadena = ""
        for c in self.cantones:
            for p in c.parroquias:
                cadena = "%s%s\n" % (cadena, p.nombre_parroquia)
        return cadena
...
class Canton(Base):
    ...
    def __repr__(self):
        return "Canton: Código División Política Administrativa  Cantón = %d Nombre Parroquia =%s" % (
...
class Parroquia(Base):
    ...
    def obtener_tipos_jornada_establecimientos(self):
        cadena = ""
        for l in self.establecimientos:
            cadena = "%sEstablecimeinto: %s - Jornada: %s\n" % (cadena, l.nombre_Institucion,l.jornada )
        return cadena
